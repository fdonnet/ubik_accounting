...
namespace Ubik.Accounting.Api.Tests.UnitTests.Features.Accounts.Commands
{
    public class DeleteAccount_Test
    {
        ...
        {
            //Arrange
            _serviceManager.AccountService.DeleteAccountAsync(_idToDelete).Returns(true);
            _serviceManager.AccountService.GetAccountAsync(_idToDelete).Returns
        ...
        public async Task Del_AccountNotFoundException_AccountIdNotFound()
        {
            //Arrange
            _serviceManager.AccountService.DeleteAccountAsync(_idToDelete).Returns(true);
            _serviceManager.AccountService.GetAccountAsync(_idToDelete).Returns(Task.FromResult<Account?>(null));
        ...
        public async Task Del_CustomValidationException_IdRequired()
        {
            //Arrange
            _serviceManager.AccountService.DeleteAccountAsync(_idToDelete).Returns(true);
            _serviceManager.AccountService.GetAccountAsync(_idToDelete).Returns(new Account() { Id = _idToDelete, Code = "test", Label = "test" });
            _command.Id = default!;
